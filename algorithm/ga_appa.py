"""
GA-APPA: Genetic Algorithm Phase 1 + Ant Colony System Phase 2
...
# from algorithm.ga_phase1 import GAPhase1, GAConfig
from algorithm.ga_phase1_update import GAPhase1, GAConfig
import random


class GAPPAAlgorithm:
    """
    GA-APPA Algorithm: Uses GA for Phase 1 (region allocation) and ACS for Phase 2 (order optimization)

    This is a modified version of the original APPA algorithm where:
    - Phase 1: GA replaces ETR-based greedy allocation
    - Phase 2: Same ACS-based order optimization as original
    """
    ...
    def __init__(
        ...
        # Track GA fitness history for analysis
        ...
        for i in range(self.num_uavs):
            for j in range(self.num_regions):
                for k in range(self.num_regions):
                    if j != k:
                        TF[i][j][k] = self.D_matrix[j][k] / \
                            self.uavs_list[i].max_velocity
        return TF

    def region_allocation_phase(self) -> Dict[int, List[int]]:
        """
        Phase 1: Region Allocation using Genetic Algorithm
        """
        # Extract data for GA
        ...
        # Run GA
        region_assignment = ga.solve()

        # Store fitness history for analysis
        self.ga_fitness_history = ga.get_fitness_history()
    ...
    def order_optimization_phase(
        ...
        """
        Phase 2: Order Optimization using Ant Colony System
        Same as original APPA algorithm
        """
        ...
        for i in range(num_regions_assigned):
            for j in range(num_regions_assigned):
                if i != j:
                    dist = self.D_matrix[assigned_regions[i]
                                         ][assigned_regions[j]]
        ...
        for iteration in range(self.max_iterations):
            ...
            for ant in range(self.num_ants):
                ...
                while unvisited:
                    ...
                    tau[current_idx][next_idx] = (
                        1 - self.rho) * tau[current_idx][next_idx] + self.rho * tau0
            ...
            if iteration_best_length < best_length:
                best_tour = [assigned_regions[idx]
                             for idx in iteration_best_tour]
    ...
    def _select_next_region(
        ...
        if q <= self.q0:
            ...
            best_idx = None
            for j in unvisited:
                value = (tau[current_idx][j] ** self.alpha) * \
                    (eta[current_idx][j] ** self.beta)
                if value > best_value:
                    best_value = value
                    best_idx = j
            return best_idx
            ...
            for j in candidates:
                prob = (tau[current_idx][j] ** self.alpha) * \
                    (eta[current_idx][j] ** self.beta)
    ...
    def solve(self) -> Dict:
        ...
        """
        # Phase 1: GA-based region allocation
        region_assignment = self.region_allocation_phase()

        # Phase 2: ACS-based order optimization
        optimized_paths = {}
        completion_times = {}
        ...
        for uav_idx in range(self.num_uavs):
            if region_assignment[uav_idx]:
                ...
                completion_times[uav_idx] = self._compute_tour_length(
                    uav_idx, optimized_tour)
            else:
                optimized_paths[uav_idx] = []
                completion_times[uav_idx] = 0.0

        max_completion_time = max(
            completion_times.values()) if completion_times else 0.0

        return {
            'paths': optimized_paths,
            'completion_times': completion_times,
            'max_completion_time': max_completion_time,
            'ga_fitness_history': self.ga_fitness_history
        }

    def get_ga_fitness_history(self) -> List[float]:
        """Get GA fitness history for convergence analysis"""
        return self.ga_fitness_history
...
if __name__ == "__main__":
    ...
    # Run GA-APPA
    ga_appa = GAPPAAlgorithm(uavs_list, regions_list, V_matrix)
    result = ga_appa.solve()

    print("\n=== GA-APPA Results ===")
    print(f"Max Completion Time: {result['max_completion_time']:.2f}")
    for uav_idx, path in result['paths'].items():
        print(
            f"UAV {uav_idx}: {path} (Time: {result['completion_times'][uav_idx]:.2f})")
